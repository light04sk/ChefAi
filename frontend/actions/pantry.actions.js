  ...
  try {
    ...
    for (const ingredient of ingredients) {
      ...
      if (response.ok) {
        ...
        // ✅ Log individual failures instead of silently skipping
        console.error(`❌ Failed to save ingredient: ${ingredient.name}`);
      }
    }
...
export async function deletePantryItem(formData) {
  try {
    ...
    // ✅ Ownership check — verify this item belongs to the current user
    const checkRes = await fetch(`${STRAPI_URL}/api/pantry-items/${itemId}`, {
      headers: {
        Authorization: `Bearer ${STRAPI_API_TOKEN}`,
      },
    });
    ...
    const ownerId = checkData.data?.owner?.id ?? checkData.data?.owner;
